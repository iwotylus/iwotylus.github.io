...
Additionally, we will grab the <mark>password policy</mark> using <mark>enum4linux-ng</mark>. The password policy is useful when conducting password spraying attacks. However, password spraying won't be my first choice here, considering there aren't many users in the domain.
