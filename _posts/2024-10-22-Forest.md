...
Scanning for all TCP ports reveals a lot of ports, many of which are common for Windows machines. Open ports inform us that firewall may not be present or is misconfigured.
...
- Presence of ports 389 and 636 suggests it's a <mark>domain controller (DC)</mark>.
...
Additionally, we will grab the <mark>Domain SID</mark> and <mark>password policy</mark> using <mark>enum4linux-ng</mark>. The password policy is useful when conducting password spraying attacks. However, password spraying won't be my first choice here, considering there aren't many users in the domain.
...
ENUM4LINUX - next generation (v1.3.4)                                                                  
 ...
 ==========================                                                                            |    Target Information    |                                                                            ==========================                                                                            [*] Target ........... 10.10.10.161                                                                    
[*] Username ......... ''                          
[*] Random Username .. 'ozllzeyp'                                                                      
[*] Password ......... ''                                                                              
[*] Timeout .......... 5 second(s)
...
The password <mark>s3rvice</mark> has been cracked. Given the password policy, we could have expected it to be cracked fairly easily. Its minimum length is set to 7 characters, and <mark>DOMAIN_PASSWORD_COMPLEX</mark> is set to <mark>false</mark>.

### WinRM

Recall that port 5985 is open, which we can utilize to gain a foothold. I will use <mark>Evil-WinRM</mark> to log in remotely.
...
We will use <mark>BloodHound</mark> to visualize the domain and its privilege escalation paths. I won’t be using <mark>BloodHound.py</mark> because, at the time of writing this post, it has certain limitations, such as the lack of support for collecting the GPO local groups.
...
The administrator's hash can be used to conduct a <mark>pass-the-hash (PtH)</mark> attack. This time, I will use <mark>psexec.py</mark> for a change, but you can stick with <mark>Evil-WinRM</mark>.
...
We finish the box by grabbing the root flag.